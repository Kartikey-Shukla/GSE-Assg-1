...
describe('Step 1 – Block Definitions', () => {
  ...
  it('rbac_role ROLE dropdown includes BROKERAGEOWNER, REALESTATEAGENT, and BUYER', () => {
    const ws = new Blockly.Workspace();
    const block = ws.newBlock('rbac_role');
    for (const value of ['BROKERAGEOWNER', 'REALESTATEAGENT', 'BUYER']) {
  ...
  it('rbac_resource RES dropdown includes CONFIDENTIALSELLERDATA, ACTIVELISTINGS, and VIRTUALTOURS', () => {
    const ws = new Blockly.Workspace();
    const block = ws.newBlock('rbac_resource');
    for (const value of ['CONFIDENTIALSELLERDATA', 'ACTIVELISTINGS', 'VIRTUALTOURS']) {
  ...
  it('rbac_rule EFFECT accepts ALLOW and DENY; ACTION accepts Delist, List, Browse', () => {
    const ws = new Blockly.Workspace();
    ...
    for (const value of ['DELIST', 'LIST', 'BROWSE']) {
...
describe('Step 2 – Per-Block Code Generation', () => {
  ...
  it('rbac_role generates a quoted string for the selected role', () => {
    const ws = new Blockly.Workspace();
    const block = ws.newBlock('rbac_role');
    block.setFieldValue('BROKERAGEOWNER', 'ROLE');
    generator.init(ws);
    const code = generator.blockToCode(block);
    generator.finish('');
    expect(
      code,
      'generator.forBlock["rbac_role"] must return the ROLE value as a quoted string, e.g. "BROKERAGEOWNER"',
    ).toMatch(new RegExp(`["']BROKERAGEOWNER["']`));
    ws.dispose();
  });
  ...
  it('rbac_resource generates a quoted string for the selected resource', () => {
    const ws = new Blockly.Workspace();
    const block = ws.newBlock('rbac_resource');
    block.setFieldValue('CONFIDENTIALSELLERDATA', 'RES');
    generator.init(ws);
    const code = generator.blockToCode(block);
    generator.finish('');
    expect(
      code,
      'generator.forBlock["rbac_resource"] must return the RES value as a quoted string, e.g. "CONFIDENTIALSELLERDATA"',
    ).toMatch(new RegExp(`["']CONFIDENTIALSELLERDATA["']`));
    ws.dispose();
  });
  ...
  it('rbac_rule with ALLOW generates return "Allow" and references the ACTION', () => {
    ...
    block.setFieldValue('DELIST', 'ACTION');
    generator.init(ws);
    const code = generator.blockToCode(block) as string;
    generator.finish('');
    expect(
      code,
      'generator.forBlock["rbac_rule"] must include the ACTION value ("DELIST") in a conditional',
    ).toMatch(new RegExp(`["']DELIST["']`));
    expect(
      code,
  ...
  it('rbac_rule with DENY generates return "Deny"', () => {
    ...
    block.setFieldValue('LIST', 'ACTION');
    generator.init(ws);
    const code = generator.blockToCode(block) as string;
    generator.finish('');
    expect(
...
describe('Step 3 – Full Policy Generation', () => {
  ...
  it('generates a function named RealEstateMlsSystem from the fixture workspace', () => {
    const ws = loadFixture('data-viewer.workspace.json');
    const code = generator.workspaceToCode(ws);
    expect(
      code,
      'Expected the workspace to produce "function RealEstateMlsSystem(role, action, resource)" — check your rbac_policy generator',
    ).toMatch(new RegExp(`function\\s+RealEstateMlsSystem\\s*\\(\\s*role\\s*,\\s*action\\s*,\\s*resource\\s*\\)`));
    ws.dispose();
  });

  it('generated code includes a role membership guard with BROKERAGEOWNER', () => {
    const ws = loadFixture('data-viewer.workspace.json');
    const code = generator.workspaceToCode(ws);
    expect(
      code,
      'The allowedRoles list must contain "BROKERAGEOWNER" as specified by the workspace fixture',
    ).toMatch(new RegExp(`["']BROKERAGEOWNER["']`));
    ws.dispose();
  });

  it('generated code includes an action+resource conditional for DELIST on CONFIDENTIALSELLERDATA', () => {
    const ws = loadFixture('data-viewer.workspace.json');
    const code = generator.workspaceToCode(ws);
    expect(
      code,
      'The generated code must check action === "DELIST" — check your rbac_rule generator',
    ).toMatch(new RegExp(`["']DELIST["']`));
    expect(
      code,
      'The generated code must include "CONFIDENTIALSELLERDATA" in the resource list — check your rbac_resource generator',
    ).toMatch(new RegExp(`["']CONFIDENTIALSELLERDATA["']`));
    ws.dispose();
  });
...
describe('Step 4 – Policy Evaluation Correctness', () => {
  ...
  it('RealEstateMlsSystem: BROKERAGEOWNER DELIST CONFIDENTIALSELLERDATA → Allow', () => {
    const ws = loadFixture('data-viewer.workspace.json');
    expect(
      runPolicy(ws, 'RealEstateMlsSystem', 'BROKERAGEOWNER', 'DELIST', 'CONFIDENTIALSELLERDATA'),
      'BROKERAGEOWNER performing DELIST on CONFIDENTIALSELLERDATA should be allowed — check that the fixture has BROKERAGEOWNER in ROLES with an ALLOW DELIST rule for CONFIDENTIALSELLERDATA',
    ).toBe('Allow');
    ws.dispose();
  });

  it('RealEstateMlsSystem: REALESTATEAGENT DELIST CONFIDENTIALSELLERDATA → Deny (role not listed)', () => {
    const ws = loadFixture('data-viewer.workspace.json');
    expect(
      runPolicy(ws, 'RealEstateMlsSystem', 'REALESTATEAGENT', 'DELIST', 'CONFIDENTIALSELLERDATA'),
      'REALESTATEAGENT is not in the policy\'s ROLES list — the role guard must return "Deny" before checking rules',
    ).toBe('Deny');
    ws.dispose();
  });

  it('RealEstateMlsSystem: BROKERAGEOWNER LIST CONFIDENTIALSELLERDATA → Deny (action not covered)', () => {
    const ws = loadFixture('data-viewer.workspace.json');
    expect(
      runPolicy(ws, 'RealEstateMlsSystem', 'BROKERAGEOWNER', 'LIST', 'CONFIDENTIALSELLERDATA'),
      'LIST is not covered by any rule in this policy — the function must fall through to the default Deny',
    ).toBe('Deny');
    ws.dispose();
  });

  it('RealEstateMlsSystem: BROKERAGEOWNER DELIST ACTIVELISTINGS → Deny (resource not covered)', () => {
    const ws = loadFixture('data-viewer.workspace.json');
    expect(
      runPolicy(ws, 'RealEstateMlsSystem', 'BROKERAGEOWNER', 'DELIST', 'ACTIVELISTINGS'),
      'ACTIVELISTINGS is not in the DELIST rule\'s resource list — the conditional must not match and must fall through to Deny',
    ).toBe('Deny');
    ws.dispose();
  });
});
