...
## Application Domain: Real Estate MLS System (RBAC)
...
1. **Roles:** Which roles are governed by this policy?
  *(Values: `BrokerageOwner`, `RealEstateAgent`, `Buyer`)*
...
2. **Action:** Operation to check.
  *(Values: `Delist`, `List`, `Browse`)*
3. **Resources:** A list of resources this rule applies to.
  *(Values: `ConfidentialSellerData`, `ActiveListings`, `VirtualTours`)*
...
## Example 1: The "RealEstateMlsSystem" Policy
**The Scenario:** `BrokerageOwner` principals may perform `Delist` on `ConfidentialSellerData`, but no other access is granted.

**The Declarative JSON Rule:**
```json
{
  "NAME": "RealEstateMlsSystem",
  "ROLES": ["BrokerageOwner"],
  "RULES": [
    {
      "EFFECT": "Allow",
      "ACTION": "Delist",
      "RESOURCES": ["ConfidentialSellerData"]
    }
  ]
}
```
...
**The Scenario:** `RealEstateAgent` and `Buyer` principals are in scope. They may perform `List` on `ActiveListings`, but `Browse` on `ActiveListings` is explicitly denied.

**The Declarative JSON Rule:**
```json
{
  "NAME": "SecureAccess",
  "ROLES": ["RealEstateAgent", "Buyer"],
  "RULES": [
    {
      "EFFECT": "Allow",
      "ACTION": "List",
...
*   **The Role Block (`rbac_role`):** Previous and next connections (type `rbac_role`), and a dropdown field `ROLE` with values `BrokerageOwner`, `RealEstateAgent`, `Buyer`.
*   **The Rule Block (`rbac_rule`):** Previous and next connections (type `rbac_rule`), a dropdown `EFFECT` (`Allow`/`Deny`), a dropdown `ACTION` (`Delist`, `List`, `Browse`), and a statement input `RESOURCES`.
*   **The Resource Block (`rbac_resource`):** Previous and next connections (type `rbac_resource`), and a dropdown field `RES` with values `ConfidentialSellerData`, `ActiveListings`, `VirtualTours`.
...
*   **`rbac_role`:** Return the `ROLE` field value as a quoted string, e.g. `"BROKERAGEOWNER"`.
*   **`rbac_resource`:** Return the `RES` field value as a quoted string, e.g. `"CONFIDENTIALSELLERDATA"`.
